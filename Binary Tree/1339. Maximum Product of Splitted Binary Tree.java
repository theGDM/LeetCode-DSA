...
class Solution {
    ...
    //Travel and Change Strategy
    public int calculateMaxProd(TreeNode root, int totalSum){
        if(root == null) return 0;
        int lsum = calculateMaxProd(root.left, totalSum);
        int rsum = calculateMaxProd(root.right, totalSum);
        int currSum = lsum + rsum + root.val;
        maxProd = Math.max(maxProd, (long)currSum * (totalSum - currSum));
        return currSum;
    }
