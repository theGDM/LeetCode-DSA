...
class Solution {
    ...
    public int maxDistance(int[][] mat) {
        ...
        //edge case, if no water is there, or only land is there
        if(q.size() == mat.length * mat[0].length || q.size() == 0) return -1; 
        ...
        while(q.size() > 0){
            ...
            ans[rem.r][rem.c] = rem.dist; //work (this statement is not necessary but can be useful)
            if(rem.dist > maxDist){
                maxDist = rem.dist;
            }
    ...
    public void addNbrs(int r, int c, int d, int[][] mat, boolean[][] visited, Queue<Pair> q){
        if(r < 0 || c < 0 || r >= mat.length || c >= mat[0].length || mat[r][c] == 1 || visited[r][c] == true){
            return;
        }
